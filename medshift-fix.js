...
/* =========================================
   ДОПОЛНЕНИЕ 28: ОТЛОЖЕННЫЕ СТАРТОВЫЕ ЗАПРОСЫ
   Убирает 3×401 на старте (meta, online, heartbeat)
   ========================================= */
(function () {
  if (window.__fix28_applied) return;
  window.__fix28_applied = true;

  // Перехват restGet/restPut только для стартовых путей
  var _restGet28 = window.restGet;
  var _restPut28 = window.restPut;
  var startPaths = ['meta', 'online'];

  function hasToken() {
    return !!(window.__fbToken || localStorage.getItem('medshift_fb_auth'));
  }

  if (_restGet28) {
    window.restGet = function (path) {
      // Если это стартовый путь и нет токена — ждём
      if (startPaths.some(function(p) { return path === p || path.endsWith('/' + p + '.json'); }) && !hasToken()) {
        console.log('[Fix28] ⏳ Отложен GET:', path, '(ожидание токена)');
        return new Promise(function(resolve) {
          var check = setInterval(function() {
            if (hasToken()) {
              clearInterval(check);
              resolve(_restGet28(path));
            }
          }, 200);
          // Таймаут 5 сек — если токен так и не появился, делаем запрос anyway
          setTimeout(function() { clearInterval(check); resolve(_restGet28(path)); }, 5000);
        });
      }
      return _restGet28(path);
    };
  }

  if (_restPut28) {
    window.restPut = function (path, data) {
      // Heartbeat (online/xxx.json) тоже ждёт токен
      if (path && path.startsWith('online/') && !hasToken()) {
        console.log('[Fix28] ⏳ Отложен PUT:', path, '(ожидание токена)');
        return new Promise(function(resolve) {
          var check = setInterval(function() {
            if (hasToken()) {
              clearInterval(check);
              resolve(_restPut28(path, data));
            }
          }, 200);
          setTimeout(function() { clearInterval(check); resolve(_restPut28(path, data)); }, 5000);
        });
      }
      return _restPut28(path, data);
    };
  }

  console.log('[Fix28] ✅ Отложенные стартовые запросы активированы');
})();
/* =========================================
   ДОПОЛНЕНИЕ 29: ПЕРЕХВАТ НАТИВНОГО FETCH
   Убирает стартовые 401 (meta, online, heartbeat)
   ========================================= */
(function () {
  if (window.__fix29_applied) return;
  window.__fix29_applied = true;

  var _nativeFetch = window.fetch;
  var startPaths = ['meta.json', 'online.json', '/online/'];
  ...
  window.fetch = function (url, opts) {
    var isStartPath = startPaths.some(function (p) {
      return typeof url === 'string' && url.indexOf(p) >= 0;
    });

    // Если это стартовый запрос и нет токена — ждём токен
    if (isStartPath && !hasToken()) {
      console.log('[Fix29] ⏳ Отложен fetch:', url);
      return new Promise(function (resolve, reject) {
        var check = setInterval(function () {
          if (hasToken()) {
            clearInterval(check);
            clearTimeout(timeout);
            resolve(_nativeFetch(url, opts));
          }
        }, 200);
        var timeout = setTimeout(function () {
          clearInterval(check);
          // Таймаут 5 сек — делаем запрос даже без токена
          resolve(_nativeFetch(url, opts));
        }, 5000);
      });
    }
  ...
  console.log('[Fix29] ✅ Перехват нативного fetch активирован');
})();
