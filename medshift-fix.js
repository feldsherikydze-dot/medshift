...
(function () {
  ...
  window.deleteMyAccountDlg = function () {
    ...
    openDlg(
      ...
      '<p style="color:var(--mut);font-size:calc(var(--fs) - 2px)">Аккаунт «<b>' + esc(u.name) + '</b>» будет удалён навсегда.</p>' +
      '<label>Введите PIN</label>' +
      '<input id="delPin" type="password" inputmode="numeric" maxlength="4" placeholder="••••" autofocus>' +
      '<p style="margin-top:10px"><button class="btn del" onclick="doDeleteMyAccount()">Удалить</button> ' +
  ...
  window.doDeleteMyAccount = function () {
    ...
    ask('Точно удалить «' + esc(u.name) + '»?', function () {
      DB.users = DB.users.filter(function (x) { return x.id !== u.id; });
      if (window.restDelete) restDelete('state/date/users/' + u.id).catch(function(){});
      DB.session = null;
  ...
  window.safeResetDlg = function () {
    var u = window.me ? me() : null;
    if (!u) return toast('Сначала войдите');
    openDlg(
      '<h3>♻️ Сброс устройства</h3>' +
      '<p style="color:var(--mut);font-size:calc(var(--fs) - 2px)">Очистит кэш <b>только на этом устройстве</b>. База на сервере НЕ удалится.</p>' +
      '<label>Введите PIN</label>' +
  ...
  window.doSafeReset = function () {
    ...
    if (window.DB) { DB.session = null; DB.users = []; DB.bags = []; DB.cars = []; DB.reports = []; DB.chat = []; }
    closeDlg(); toast('♻️ Устройство сброшено'); setTimeout(function(){ location.reload(); }, 800);
  };
  ...
  var checkInterval = setInterval(function () {
    if (window.syncPush && !window.__syncPushWrapped) {
      ...
      console.log('[Fix26] ✅ syncPush безопасно обёрнут');
      clearInterval(checkInterval);
    }
  }, 500);

  // Остановка проверки через 10 секунд, чтобы не гонять интервал вечно
