...
/* =========================================
   ДОПОЛНЕНИЕ 25: ИСПРАВЛЕНИЕ ПУТИ USERS
   state/users → state/date/users
   ========================================= */
(function () {
  if (window.__fix25_applied) return;
  window.__fix25_applied = true;

  var _restGet = window.restGet;
  if (_restGet) {
    window.restGet = function (path) {
      if (path === 'state/users' || path === 'users') {
        console.log('[Fix25] 🔄 GET:', path, '→ state/date/users');
  ...
  if (_restPut) {
    window.restPut = function (path, data) {
      if (path === 'state/users' || path === 'users') {
        console.log('[Fix25] 🔄 PUT:', path, '→ state/date/users');
  ...
  if (_restDelete) {
    window.restDelete = function (path) {
      if (path && (path.startsWith('state/users/') || path.startsWith('users/'))) {
        ...
        console.log('[Fix25] 🔄 DELETE:', path, '→ state/date/users/' + uid);
  ...
  console.log('[Fix25] ✅ Перехват путей users активирован (state/date/users)');
})();
