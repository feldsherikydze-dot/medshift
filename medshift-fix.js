...
/* =========================================
   ДОПОЛНЕНИЕ 21: удаление своего аккаунта
   с подтверждением через PIN
   ========================================= */
(function () {
  if (window.__fix21_applied) return;
  window.__fix21_applied = true;

  window.deleteMyAccountDlg = function () {
    var u = me();
    if (!u) return toast('Вы не вошли');

    openDlg(
      '<h3>🗑 Удалить мой аккаунт</h3>' +
      '<p style="font-size:calc(var(--fs) - 2px);color:var(--mut)">Это действие необратимо. Все ваши данные (отчёты, чаты) останутся в базе, но доступ будет закрыт.</p>' +
      '<label>Введите ваш PIN для подтверждения</label>' +
      '<input id="delMyPin" type="password" inputmode="numeric" maxlength="4" placeholder="••••">' +
      '<p><button class="btn del" onclick="deleteMyAccountDo()">Удалить навсегда</button> ' +
      '<button class="btn sec" onclick="closeDlg()">Отмена</button></p>'
    );
  };

  window.deleteMyAccountDo = function () {
    var u = me();
    if (!u) return;

    var pin = document.getElementById('delMyPin').value;
    if (pin !== u.pin) {
      toast('❌ Неверный PIN');
      return;
    }

    ask('Точно удалить аккаунт «' + esc(u.name) + '»? Это нельзя отменить.', function () {
      ...
      // Удаляем со склада (если есть токен)
      if (window.__fbToken && window.restDelete) {
        restDelete('state/users/' + u.id).catch(function () {});
      }
  ...
  // Добавляем кнопку в раздел «Ещё»
  if (window.setView) {
    var _sv21 = window.setView;
    window.setView = function () {
      var h = _sv21();
      // Вставляем кнопку перед кнопкой выхода
      return h.replace(
        '<button class="btn del" onclick="logout()">🚪 Выйти</button>',
        '<button class="btn del" onclick="deleteMyAccountDlg()">🗑 Удалить мой аккаунт</button> ' +
        '<button class="btn del" onclick="logout()">🚪 Выйти</button>'
      );
    };
  }
