import { DB, save, uid, me, isBoss, esc, todayStr, daysLeft, normName, isDead, isBannedName, purgeSched, bdayToday } from './db.js';
...
function footLine() {
  const n = window.DRUG_DB ? window.DRUG_DB.length : 0;
  return '<p style="text-align:center;font-size:calc(var(--fs) - 4px);color:var(--mut);margin-top:10px">MedShift · версия v'
    + VERSION + ' · справочник: ' + (n ? n + ' преп. 💊' : '⚠ справочник не загружен') + '</p>';
}
