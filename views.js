...
export let tab = 'home';
export let loginFor = null;
export let curBag = null, curCar = null, curKit = null;
export let curTpl = false, curPot = false, curPotKit = null;
export let openTplId = null, tplSearch = '', refSearch = '';
export let schedSub = 'days';
export let highlightItem = null;
export let editBag = null, editKit = null, editEquip = null, editTpl = null;

export function go(t) {
  tab = t;
  localStorage.setItem('medshift_tab', t);
  if (t !== 'bags') { curTpl = false; curPot = false; curPotKit = null; openTplId = null; tplSearch = ''; }
  if (t !== 'ref') refSearch = '';
...
function calGrid() {
  ...
  let h = '<div class="cal">';
  ['пн','вт','ср','чт','пт','сб','вс'].forEach(d => h += '<div class="hd">' + d + '</div>');
  for (let i = 0; i < first; i++) h += '<div></div>';
  for (let d = 1; d <= days; d++) h += '<div class="' + (d === n.getDate() ? 'td' : '') + '">' + d + '</div>';
  return h + '</div>';
}
...
export function bagView() {
  const b = DB.bags.find(x => x.id === curBag);
  if (!b) { curBag = null; return bagsView(); }
  const em = editBag === curBag;
  ...
  (b.items || []).forEach((it, i) => {
    const hl = highlightItem && it.name === highlightItem ? ' highlight' : '';
    ...
    if (em) {
      ...
      h += '<tr class="' + rc + hl + '" data-item="' + esc(it.name) + '"><td>' + (it.potent ? '⚕ ' : '') + esc(it.name) + '</td><td>' + esc(it.spec || '') + '</td><td>' + it.qty + ' ' + esc(it.unit) + '</td><td><input type="date" value="' + (it.expiry || '') + '" data-act="setExp" data-arg="' + curBag + ',' + i + '"></td><td><button class="btn del" data-act="delItem" data-arg="' + curBag + ',' + i + '">🗑</button></td></tr>';
    }
  });
  h += '</tbody></table>';
  if (highlightItem) setTimeout(() => highlightItem = null, 4500);
  return h;
}
...
export function carView() {
  const c = DB.cars.find(x => x.id === curCar);
  if (!c) { curCar = null; return carsView(); }
  const em = editEquip === curCar;
...
function kitView(c) {
  const k = c.kits.find(x => x.id === curKit);
  if (!k) { curKit = null; return carView(c); }
  const em = editKit === curKit;
...
export function refView() {
  ...
  h += '<div class="refSearch"><input type="text" id="refInput" placeholder="🔍 Поиск препарата…" value="' + esc(refSearch) + '" oninput="window.__refSearchInput(this.value)" autofocus>';
...
export function schedView() {
  ...
  h += '<button class="btn ' + (schedSub === 'days' ? '' : 'sec') + '" data-act="schedDays">📷 Журнал смены</button>';
  h += '<button class="btn ' + (schedSub === 'months' ? '' : 'sec') + '" data-act="schedMonths">📅 График месяца</button>';
  h += '<button class="btn ' + (schedSub === 'grid' ? '' : 'sec') + '" data-act="schedGrid">🗓 Смены</button>';
  h += '</div>';
  if (schedSub === 'grid') return h + shiftGridView();
  if (schedSub === 'days') {
    h += '<p><label class="btn sec" style="display:inline-block">📷 Фото<input type="file" hidden accept="image/*" data-kind="days" onchange="window.__addSchedPhoto(this,\'days\')"></label></p>';
    h += '<p style="font-size:calc(var(--fs) - 2px);color:var(--mut)">Фото журнала хранятся 2 дня.</p>';
    if (DB.sched.days.length) {
      h += '<div class="thumbs">' + DB.sched.days.map(p => '<img src="' + p.img + '" data-act="openPhoto" data-arg="days,' + p.id + '">').join('') + '</div>';
    } else h += '<p>Фото журнала пока нет.</p>';
    return h;
  }
  h += '<p><label class="btn sec" style="display:inline-block">📷 Фото<input type="file" hidden accept="image/*" onchange="window.__addSchedPhoto(this,\'months\')"></label></p>';
  ...
  if (DB.sched.months.length) {
    h += '<div class="thumbs">' + DB.sched.months.map(p => '<img src="' + p.img + '" data-act="openPhoto" data-arg="months,' + p.id + '">').join('') + '</div>';
...
export function setView() {
  ...
  h += '<label>Цвет темы</label><input type="color" value="' + esc(DB.settings.accent) + '" data-act="setAccent">';
