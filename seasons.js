...
export const HOLS = {
  ...
  '06-12':'🇷 День России','09-01':'🎓 День знаний','11-04':'🤝 Народное единство',
  '12-31':'🎆 Канун Нового года'
};
export function holOf(d) {
  const key = ('0' + (d.getMonth() + 1)).slice(-2) + '-' + ('0' + d.getDate()).slice(-2);
  if (HOLS[key]) return HOLS[key];
  if (d.getMonth() === 5 && d.getDay() === 0 && d.getDate() >= 15 && d.getDate() <= 21) return '💚 День медицинского работника';
  return '';
}
