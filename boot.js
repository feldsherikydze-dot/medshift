...
window.biometricLogin = async () => { const u = await biometricLogin(); if (!u) return u; go(tab); announceLogin(u); presBeat(); toast('Смена начата 👋'); return u; };
...
window.addBagTpl = () => {
  ...
  DB.bagTypes = [{ id: 1, name: 'Шаблон сумки (стандарт)', items: window.SEED.map(r => ({ name: r[2], spec: r[3], unit: r[4], qty: r[5], expiry: '', potent: false })) }];
...
window.loginByName = async () => {
  ...
  if (!u) return toast('Не найден. Если аккаунт создан в другом браузере — там должен быть введён «Ключ склада», иначе он остался только на том устройстве');
...
window.showAlertsDlg = () => {
  ...
  a.forEach((al, ai) => {
    h += '<div class="card" style="margin-bottom:8px"><span class="badge ' + al.l + '">!</span> <b>' + esc(al.t) + '</b>';
    if (al.items && al.items.length) {
      h += '<table style="margin-top:6px"><tr><th>Объект</th><th>Позиция</th><th>Срок</th><th></th></tr>';
      al.items.forEach((it, i) => {
        const label = it.type === 'bag' || it.type === 'pot' ? it.name : it.carName + ' / ' + it.kitName;
        h += '<tr><td>' + esc(label) + '</td><td>' + esc(it.item.name) + '</td><td>' + badgeExp(it.item.expiry) + '</td><td><button class="btn sec mini" data-act="navAlert" data-arg="' + ai + ',' + i + '">→</button></td></tr>';
      });
      h += '</table>';
    } else if (al.reportId) {
