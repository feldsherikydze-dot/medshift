...
export function banUser(id) { if (String(id) === String(DB.session)) throw new Error('Нельзя себя'); const u = DB.users.find(x => String(x.id) === String(id)); if (!u) throw new Error('Не найден'); DB.bans = DB.bans || []; DB.bans.push(u); DB.users = DB.users.filter(x => String(x.id) !== String(id)); save(); }
